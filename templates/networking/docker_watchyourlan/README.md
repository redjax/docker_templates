...
Back in the WatchYourLAN UI, open the config and fill in the details under "InfluxDB2 config." For address, use the machine's IP; if you try to use the `influxdb` container's DNS name, it won't work because the WatchYourLAN container is using `network_mode: "host"` configuration.
