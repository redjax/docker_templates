...
from metadata.constants import TEMPLATES_ROOT, OUTPUT_DIR, TEMPLATE_INDICATORS, TEMPLATE_BEACONS, CATEGORIES_METADATA_FILE, IGNORE_COUNT_PATTERNS_FILE
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        default=IGNORE_COUNT_PATTERNS_FILE,
        help="File with directory/filename patterns to ignore",
    )
...
def count(
    ...
    ignore_patterns_file: list[str] | None = None,
    ...
    if ignore_patterns_file is None:
        ignore_patterns = []
    else:
        ignore_patterns = load_ignored_patterns(ignore_patterns_file=ignore_patterns_file)
...
if __name__ == "__main__":
    ...
    try:
        templates_count: int | None = count(
            ...
            ignore_patterns_file=args.ignore_patterns_file or IGNORE_COUNT_PATTERNS_FILE,
            template_file_indicators=TEMPLATE_BEACONS["docker_template"],
